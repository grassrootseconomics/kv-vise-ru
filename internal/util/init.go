...
func InitConfig(lo *slog.Logger, confFilePath string) *koanf.Koanf {
	...
	err := ko.Load(env.ProviderWithValue("KVVISERU_", ".", func(s string, v string) (string, interface{}) {
		key := strings.ReplaceAll(strings.ToLower(strings.TrimPrefix(s, "KVVISERU_")), "__", ".")
